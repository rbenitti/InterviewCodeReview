Errores 
	En el método LogMessage, se repite el nombre de los parámetros string message y bool message 
...
Estilo
	...
	Se levantan excepciones genéricas.
...
Diseño
	...
	La clase JobLogger tiene demasiadas responsabilidades, lo que la vuelve compleja, dificil de modificar y dificil de testear. Actualmente, la clase se encarga de:
			La lógica para decidir que tipo de logueo utilizar
			La lógica para formatear los mensajes de acuerdo al tipo y destino
			La lógica para impactar los mensajes en los distintos destinos
	El diseño es difícil de testear, pues se necesitan que muchas condiciones se cumplan para que la clase pueda utilizarse (las AppSettings deben estar instanciadas, debe existir una conexión a base de datos, connectionString debe estar definida y ser valida)
	Logueo en archivo: En lugar de realizar un Append al final del archivo, se intentar cargarlo completamente en la variable l y volver a escribirlo entero. Esto podría generar problemas como, por ejemplo, que se pisen distintos logueos.
	La configuración de logueo (nombres de archivos, colores, codigos asignados a cada tipo de mensaje) está incrustada en la clase, por lo que habría que modificar su código fuente para poder modificarla.
...
Cambios:
	...
	Hacer de JobLogger un singleton, para que la configuración de logueo sea consistente a lo largo de toda la aplicación.
	...
	Parametrizar la sentencia SQL, a fin de evitar la injección de SQL malicioso.
	Proporcionar formas de configurar JobLogger y los loggers sin modificar sus códigos internos.